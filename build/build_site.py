...
def _lb_spalten():
    ...
    Aus dem TEI gezählt, nicht gesetzt: jede Spalte trägt dort ein <cb>. Die Zahl steht in
    Aussagen wie »kommt in keiner der N Spalten vor« und muss sich mit dem Bestand bewegen."""
    n = 0
    for f in glob.glob(os.path.join(REPO, "tei", "limesblatt-*.xml")):
        try:
            n += open(f, encoding="utf-8").read().count("<cb ")
        except OSError:
            return "?"
...
def _orl_baende():
    ...
    Nicht gezählt aus einer Bandliste, sondern aus den Bandmarken des Gesamtregisters: das
    ist dieselbe Grundlage, über die die Seiten reden, und sie bewegt sich mit, wenn das
    Workset wächst. Die Zahl stand an sechs Stellen fest eingetippt."""
    r = _datenwert("orl_register", vorgabe={})
    if not isinstance(r, dict):
        return "?"
    b = set()
    for e in (r.get("persons") or []) + (r.get("places") or []):
        b.update(e.get("bands") or [])
    return len(b) or "?"


def _orl_nerzeilen():
    """Belegzeilen des ORL-Gesamtregisters (Name × Band), mit Tausenderpunkt.

    Vorher stand hier die Zeilenzahl des HathiTrust-NER-Rohexports (130.540). Die ist
    richtig, aber auf der Seite nicht nachprüfbar: der Export bleibt aus Lizenzgründen
    draußen. Gezählt wird deshalb, was der Leser vor sich hat."""
    r = _datenwert("orl_register", vorgabe={})
    if not isinstance(r, dict):
        return "?"
    n = sum(len(e.get("bands") or []) for e in (r.get("persons") or []) + (r.get("places") or []))
    return _tausend(n) if n else "?"
...
def _tausend(n):
    ...
    NICHT über `str.replace(",", ".")` auf dem fertigen Satz: das trifft auch die
    Satzkommas und machte aus »43 Aufsätze, 389.430 Wörter« ein »43 Aufsätze. 389.430 Wörter«."""
    return f"{n:,}".replace(",", ".")
...
def persons_page(persons, occ, digs):
    ...
    return (f'<h1>Personenregister</h1><p class="meta">{len(persons)} kuratierte Personen der RLK-Forschungs'
            ...
            f'<div class="note"><p><b>Was „kuratiert" hier heißt.</b> Aufgenommen ist, wer eine eigene Notiz im '
            f'Arbeitsbestand dieser Edition hat: Mitglieder und Herausgeber der Kommission, Streckenkommissare, '
            f'Ausgräber, dazu Personen des Umfelds, an denen die Forschungsgeschichte hängt. Das ist eine '
            f'<b>Entscheidung, kein Befund</b>: Wer hier fehlt, ist damit nicht unwichtig, sondern noch nicht '
            f'erschlossen. Der maschinelle Gegenpol ist das <a href="namen.html">Namenregister</a>, das jeden '
            f'gefundenen Namen führt, ohne über ihn zu urteilen. Die Spalte <b>Belege</b> zählt Fundstellen im '
            f'Limesblatt, nicht Bedeutung: wer selten schrieb und viel entschied, steht unten.</p></div>'
...
def places_page(places, occ, pname, str_by_id, sites, site_hits):
    ...
    body = (f'<h1>Ortsregister</h1><p class="meta">{len(places)} benannte Kastelle (Karten unten) plus '
            ...
            f'<div class="note"><p><b>Auf dieser Karte darf nichts gemessen werden.</b> Die Kastellpunkte sind '
            f'gegengeprüft (Wikidata gegen die Digital Atlas of the Roman Empire, verbunden über die DARE-ID, '
            f'nicht über Namensgleichheit): 18 von 20 bestanden, Abweichung 0–79&#8239;m, im Mittel 5&#8239;m. '
            f'Zwei Kastelle sind nicht prüfbar, weil ihre Notiz <i>zwei</i> Plätze behandelt (Öhringen, '
            f'Welzheim) und nur einen Punkt führt. Übereinstimmung ist dabei kein Beweis: beide Quellen speisen '
            f'sich teils aus denselben Angaben. Und <b>was der Punkt bezeichnet, ist nirgends festgehalten</b>: '
            f'Kastellmitte, Ortsmitte oder Fundstelle.</p>'
            f'<p>Die <b>Strecken tragen mit Absicht keine Koordinate</b>: ein Abschnitt ist ein Verlauf, kein '
            f'Punkt, und ein Marker müsste einen Mittelpunkt erfinden, den die Quelle nicht nennt. Die '
            f'{len(sites)} weiteren Stellen stammen aus DARE und sind hier <b>nicht einzeln nachgeprüft</b>; sie '
            f'zeigen, wo eine Anlage verzeichnet ist, nicht, wie genau.</p></div>'
...
def strecken_page(strecken, str_forts, persons, pname, strecke_sites, orl_idx, volumes, abta=None, slit=None):
    ...
    nur von Ort zu Ort („Miltenberg–Rehberg") und belegte das mit der nackten Zeile
    „ORL Abteilung A, Strecke 7": ein Beleg aus dem eigenen Bestand, also keiner. Die echten
    Faszikeltitel stehen im Verbundkatalog, und sie benennen anders. Sie stehen jetzt bei
    jedem Abschnitt, den sie belegen."""
    byname = {p["name"]: p for p in persons}
    # Die Titel stehen in `vergleich` (Vault-Titel gegen RLK-Titel), nicht in `records`
    # (das sind die Faszikel als bibliographische Einheiten, oft mehrere Strecken je Band).
    abtitel = {}
    for r in (abta or {}).get("vergleich", []):
    ...
    for s in strecken:
        ...
        ra = abtitel.get(nr)
        if ra:
            jahr = f' <span class="meta">({html.escape(str(ra["orl_jahr"]))})</span>' if ra.get("orl_jahr") else ""
            extra += (f'<div class="x rlktitel">📜 <b>So nennt ihn die RLK selbst:</b> '
                      f'„{html.escape(ra["orl_titel"])}"{jahr}</div>')
    ...
    stand = (abta or {}).get("stand", {})
    mit = stand.get("strecken_mit_orl_titel", len(abtitel))
    samml = stand.get("sammelfaszikel", [])
    ohne = stand.get("ohne_orl_titel", [])
    samml_txt = ", ".join("+".join(str(x) for x in p) for p in samml)
    return (f'<h1>Strecken</h1><p class="lede">{len(strecken)} Limes-Abschnitte, je mit den Kastellen, den '
            f'zugehörigen <b>Limesblatt-Bänden</b> (Vorbericht) und der <b>ORL</b>-Endpublikation nebeneinander, '
            f'den beteiligten Personen und den DARE-Stellen entlang der Linie.</p>'
            f'<div class="note"><p><b>Die Überschriften hier sind nicht die der Kommission.</b> Ein Abschnitt '
    ...
    teile = [
        ...
        block("orl", "Obergermanisch-Raetischer Limes (ORL)", "1894–1937 · Endpublikation",
              ...
              [("Inhaltsverzeichnis", "register/orl-inhalt.html"), ("Bandindex", "register/orl.html"),
               ...
               ("Auswertungen &amp; Daten", "register/hathitrust.html#auswertungen")]),
        ...
        block("lokal", "Lokale &amp; regionale Publikationen", "das föderale Ökosystem",
              ...
              [f"<b>{len(organe)} Organe</b> im ORL-Apparat vollständig ausgezählt",
               ...
               "das Ökosystem wuchs zwischen Vorbericht und Endpublikation, denn die Mainzer Zeitschrift "
        ...
        block("aufsaetze", "Einzelne Aufsätze", "Nachrufe · Streitschriften · Gründungstexte",
              ...
              [f'<b>{len(artliste or [])} Aufsätze</b> erschlossen, '
               f'{_tausend(sum(x.get("woerter", 0) for x in (artliste or [])))} Wörter',
...
def baende_page(volumes, toc=None):
    ...
    toc = toc or {}
    # Die acht Bände standen als eine einzige Liste ohne eine einzige Überschrift auf der
    # Seite: 291 Listenpunkte, kein Sprungziel, und „Band 5" war nicht verlinkbar. Jeder
    # Band bekommt jetzt seinen eigenen Abschnitt mit Anker, darüber eine Sprungleiste.
    bl, sprung = [], []
    for v in volumes:
        ents = toc.get(v["nr"], [])
        ...
        anker = f'bd{v["nr"]}'
        sprung.append(f'<a href="#{anker}">Bd.&#160;{v["nr"]}</a>')
        bl.append(f'<section class="bandblock" id="{anker}">'
                  f'<h2><a href="volumes/bd{v["nr"]}.html">{html.escape(v["label"])}</a></h2>'
                  f'<p class="meta">{len(v["pages"])} Seiten{spanne} · {len(ents)} Berichte · '
                  f'<a href="{BASE_DIGLIT}/{html.escape(v["slug"])}">Digitalisat der UB Heidelberg</a></p>'
                  f'{sub}</section>')
    sprungleiste = f'<p class="bandnav">Zum Band: {" · ".join(sprung)}</p>'
    n_ber = sum(len(x) for x in toc.values())
    return (f'<h1>Limesblatt: Bände &amp; Inhaltsverzeichnis</h1>'
            f'<p class="lede">Das Feldorgan der Reichs-Limeskommission, vollständig: {len(volumes)} '
            f'Jahrgangsbände, {len(HEFTE)} Hefteinheiten, {n_ber} nummerierte Feldberichte. Jeder Bericht führt an seine '
            ...
            f'[1903]</i>". <span class="lc">(Eckige Klammern stehen für Lesungen, wo die Fraktur-OCR versagt: sie liest '
...
def index_page(volumes, toc=None, zahlen=None):
    ...
    body = (f'<h1>RLK-digital</h1>'
            f'<p class="lede">Die <b>Reichs-Limeskommission</b> (1892–1937) war das erste länderübergreifende '
            f'Großforschungsunternehmen des Kaiserreichs. Hier stehen ihre Quellen: das Feldorgan <em>Limesblatt</em>, '
            f'die Endpublikation <em>Obergermanisch-Raetischer Limes</em>, die Jahresberichte der Kommission, '
            f'die zitierte Literatur und die Archivbestände, die dahinter liegen.</p>'
...
def analysis_sections(volumes, orl_lex=None):
    ...
    if os.path.exists(orlp):
        ...
        out.append('<h2 id="orl">Einzelprobe an einem Ort: Osterburken 1895</h2>'
                   ...
                   'Belegstellen 0× Holzbau, 0× Erdkastell, 0× Steinbau, Oberscheidental 1935 dagegen in 591 '
...
def gesamtbibliographie_page(bibls, idx, abta, jb, rez, zs, bli, hefte):
    ...
    return (f'<h1>Gesamtbibliographie</h1>'
            ...
            f'<b>{len(rows)} Titel</b> aus sechs getrennt gepflegten Registern, nämlich der im Limesblatt zitierten '
...
def _korpus(i, feld):
    """Korpus-Nummern einer Inschrift (`cil` oder `brambach`), fett wo der ORL sie zitiert.

    Bewusst ohne Link: das Ziel wäre der Scan des ORL-Bandes bei HathiTrust, und die Nummer
    steht hier als Zitierform, nicht als Wegweiser in ein Digitalisat. Die Fundstelle nennt
    ...
    for n in c:
        o = orl.get(n)
        if o:
            t = html.escape(("Im ORL %d\u00d7 zitiert \u00b7 %s" % (o.get("n", 1), o.get("ort", ""))).strip(" \u00b7"))
            teile.append(f'<b title="{t}">{html.escape(n)}</b>')
        else:
            teile.append(html.escape(n))
    return " \u00b7 ".join(teile)
...
def inscriptions_page(edh):
    ...
    for k in edh.get("kastelle", []):
        ...
        rows = "".join(
            ...
            + (' <span class="meta">[' + " · ".join(t for t in (_korpus(i, "cil"), _korpus(i, "brambach")) if t) + ']</span>'
               if (i.get("cil") or i.get("brambach")) else "")
    ...
    for k in edh.get("kastelle", []):
        ...
        for i in k["inschriften"]:
            alle.append(
                ...
                f'<td>{_korpus(i, "cil") or "—"}</td>'
                f'<td>{_korpus(i, "brambach") or "—"}</td>'
    ...
    tabelle = (f'<h2 id="tabelle">Alle Inschriften in einer Tabelle</h2>'
               ...
               f'<th>Inschrift</th><th>CIL</th><th>Brambach</th><th>EDH</th></tr></thead><tbody>{"".join(alle)}</tbody></table>')
    return (f'<h1>Inschriften (EDH)</h1>'
            f'<p class="meta">{edh.get("total", 0)} Inschriften von {len(edh.get("kastelle", []))} '
            f'Limes-Fundorten, aus den EpiDoc-Dumps der '
            f'<a href="https://edh.ub.uni-heidelberg.de/">Epigraphic Database Heidelberg</a> (CC BY-SA), '
            f'mit Gattung, Datierung und Direktlink ins EDH. Neben dem '
            f'<a href="fundindex.html">Fundindex</a>, der die Sachfunde führt, und verknüpft mit dem '
            f'<a href="places.html">Ortsregister</a>.</p>'
            f'<p>Die Spalten <b>CIL</b> und <b>Brambach</b> nennen die Corpusnummern. Unter ihnen zitieren '
            f'die Quellen: Limesblatt und <a href="orl.html">ORL</a> führen eine Inschrift nach Corpus '
            f'und Nummer, nicht nach der HD-Nummer, die 1892 noch nicht bestand. Eine CIL-Nummer haben '
            f'{edh.get("n_cil", 0)} der Inschriften, eine bei Brambach {edh.get("n_bram", 0)}. Fett gesetzt '
            f'sind die Nummern der {edh.get("n_orl", 0)} Inschriften, die im '
            f'<a href="orl-register.html">ORL-Apparat</a> vorkommen; der Titel des Eintrags nennt '
            f'Häufigkeit und Band.</p>'
            f'<div class="note"><p><b>Zum Zuschnitt.</b> Verzeichnet ist der EDH-Bestand zu den '
            f'Fundorten, für die diese Edition eine Ortsnotiz führt, nicht die Epigraphik des Limes. '
            f'Fundorte ohne Notiz fehlen. Gattung und Datierung stammen unverändert aus der EDH; wo sie '
            f'dort offen bleiben, steht ein Strich.</p>'
            f'<p><b>Zum Limesblatt.</b> Es nennt die Corpora in 267.000 Wörtern vierzigmal, und keine '
            f'dieser Stellen betrifft eine hier verzeichnete Inschrift. Die fünfzehn CIL-Stellen führen '
            f'Vergleichsmaterial aus Afrika, Spanien, Britannien, Italien und Pannonien an; die '
            f'dreiundzwanzig Brambach-Stellen liegen im selben Zahlenbereich wie die hiesigen '
            f'(1728 bis 1757 gegen 1702 bis 1710), meinen aber andere Steine. Das Feldorgan meldet seine '
            f'Inschriften und zitiert sie nicht; der Apparat entstand bei der Endredaktion des ORL.</p>'
            f'<p><b>Zur Konkordanz.</b> Die Corpusnummern stehen in der Bibliographie der EpiDoc-Sätze, '
            f'nicht im EDH-Index. Brambach führt die EDH selten und nie als Kurzzitat, sondern als volle '
            f'Titelangabe mit nachgestellter Nummer, weshalb die Spalte meist leer bleibt. Für den '
            f'Abgleich mit dem ORL sind zwei Zitierweisen zu vereinheitlichen: die EDH zählt arabisch mit '
            f'Komma (CIL 13, 11766), der ORL römisch ohne Komma (CIL XIII 11766). CIL XIII und XVII '
            f'erschienen in Teilen, welche die EDH mitzitiert (»CIL 13, 04, 06548« steht für Band 13, '
            f'Teil 4, Nummer 6548).</p></div>'
...
def orl_page(idx, lex, bli=None, abta=None, bgl=None):
    ...
    def brow(r):
        ...
        if lf:
            ...
            # Der Verbundkatalog kennt das Jahr, aber keine Lieferungsnummer. Sein Satz gilt dem
            # EINZELFASZIKEL (»Das Kastell X«, $n = ORL-Nr.), nicht dem gebundenen Band, und trifft
            # in allen acht vergleichbaren Fällen Mertens Datum aufs Jahr.
            lfgc = (f'<td>—</td>{_orl_datum_td(r["year_k10"])}'
                    '<td class="meta">Verbundkatalog</td>')
        else:
        ...
        return (f'<tr id="orl-{html.escape(r["nr"])}"><td>{html.escape(r["nr"])}</td><td>{html.escape(r["kastell"])}{links}</td>'
                ...
                f'{_orl_band_td(r["nr"], nr2band, band_fertig)}'
    ...
    return (f'<h1>ORL: Der obergermanisch-raetische Limes des Römerreiches</h1>'
            ...
            f'sie die <i>Lieferung</i> meint: für {_bl_fehl()} Bände war sie damit falsch, und selbst bei richtiger '
            ...
            f'<th>Lieferung</th><th>Beleg</th><th>Sammelband</th><th>Vorb.</th>'
            f'<th>Bearbeiter</th></tr></thead>'
...
def hintzelmann_page(volumes):
    ...
    # deshalb zu echten Überschriften gemacht, der Wortlaut bleibt der des Drucks.
    teile = [("teil-1", r"<p>\s*I\. Verzeichnis der Mitarbeiter\.\s*</p>", "I. Verzeichnis der Mitarbeiter"),
             ("teil-2", r"<p>\s*II\. Ortsverzeichnis\.", "II. Ortsverzeichnis"),
             ("teil-3", r"<p>\s*III\. Inschriften\s*</p>", "III. Inschriften")]
    nav, gefunden = [], 0
    for anker, muster, titel in teile:
        m = re.search(muster, body)
        if not m:
            continue
        gefunden += 1
        kopf = f'<h2 id="{anker}" class="regteil">{titel}</h2>'
        rest = "" if m.group(0).endswith("</p>") else "<p>"      # Teil II trägt Text im selben Absatz
        body = body[:m.start()] + kopf + rest + body[m.end():]
        nav.append(f'<a href="#{anker}">{titel}</a>')
    navbar = (f'<p class="regnav">{" · ".join(nav)}</p>') if gefunden == 3 else ""
    # Buchstabensprung ins Ortsverzeichnis. Gearbeitet wird auf den vorhandenen Ortslinks,
    # das Register wird NICHT neu in Lemmata zerlegt (die Zeilenumbrüche des Drucks laufen
    # mitten durch die Einträge). Welche Strecke das Ortsverzeichnis ist, entscheidet die
    # längste aufsteigende Kette der Lemmaköpfe, nicht die Überschrift: die steht im
    # Textstrom an der falschen Stelle, s. den Hinweis unten.
    az = ("".join(f'<button type="button" data-b="{c}">{c}</button>'
                  for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ")) if gefunden == 3 else ""
    azbar = (f'<div class="azbar" id="azbar" hidden><span class="meta">Ortsverzeichnis:</span>{az}</div>'
             if az else "")
...
def orl_toc_page(idx, bli=None, fasz=None, dseiten=None, abta=None, places=None, bgl=None):
    ...
    for r in sorted(b, key=lambda x: int(re.sub(r"\D", "", str(x.get("nr") or "0")) or 0)):
        ...
        dat = _orl_datum_td(lf.get("jahr") or r.get("year_k10"))
        trows.append(
            ...
            f'{_orl_band_td(r.get("nr"), nr2band, band_fertig)}'
    ...
    tab_b = (f'<h2 id="tabelle">Alle Faszikel in einer Tabelle</h2>'
             ...
             f'das Suchfeld filterbar. <b>Lieferung</b> nennt den Monat, wo Merten ihn nennt, sonst '
             f'das Jahr. <b>Sammelband</b> nennt den Band und das Jahr, in dem er frühestens '
             f'vollständig war: das seines letzten Faszikels. <b>Vorb.</b> = '
             f'Zahl der zugehörigen Limesblatt-Vorberichte.</p>'
             f'<table class="reg"><thead><tr><th>ORL-Nr.</th><th>Kastell</th><th>Ort</th>'
             f'<th>Bearbeiter</th><th>Lfg.</th><th>Lieferung</th><th>Sammelband</th>'
             f'<th>Vorb.</th></tr></thead>'
    ...
    return (
        ...
        f'({n_lief} Lieferungen), nämlich Hettners Herausgeberliste bei Merten&nbsp;2002, die '
...
def genese_page(bv, bl, lat, zj, hefte, toc):
    ...
    s3 = (
        ...
        f'Druckerei Petters auf die Orthographie von 1901 um, abrupt und nicht gleitend.</p>')
...
def orl_verweise_page(bl, bv):
    ...
    return (
        ...
        f'(1894–1903) enthält einen werkinternen Verweis, alle 13 späten (1914–1937) dagegen schon '
...
def orl_apparatus_page(reg, idx, persons=None):
    ...
    persons_html = (
        ...
        f'sind <b>#Bd.</b> und <b>Nenn.</b>: Keine der {_orl_nerzeilen()} Belegzeilen fällt weg, und die '
        f'Abbildung ist injektiv ({_orl_baende()} Bände → {_orl_baende()} Nummern), sie zählt also richtig, in <i>wie vielen</i> Bänden ein Name '
    ...
    return (f'<h1>ORL-Gesamtapparat</h1>'
            ...
            f'„v.&#8201;N" als Kastell-Nummer, obwohl sie die <i>Lieferung</i> meint: für {_bl_fehl()} Bände war sie '
...
# --- Was aus der Kapsel geworden ist ----------------------------------------
# Der Export selbst ist nicht öffentlich (HathiTrust erlaubt nur nicht-konsumtive Nutzung), die
...
def kapsel_liste():
    ...
    for gruppe, eintraege in kapsel_auswertungen():
        zeilen = "".join(
            ...
            f'<td><a href="../data/{datei}"><code>{datei}</code></a></td></tr>'
            for datei, frage, befund, grenze in eintraege)
    ...
    return (f'<h2 id="auswertungen">Die {n} Auswertungen und ihre Daten</h2>'
            ...
            f'<b>abgeleiteten</b> Auswertungen dürfen es, und sie stehen hier vollständig, jede mit '
            f'der Frage, die sie beantwortet, dem Ergebnis, ihrer Grenze und der Datei zum '
            f'Nachrechnen (JSON, CC&nbsp;BY&nbsp;4.0).</p>' + "".join(teile))
...
def hathitrust_page(idx, reg, lex):
    ...
    return (f'<h1>Erschließung über HathiTrust</h1>'
            f'<p class="meta">Wie der ORL token-frei und <b>nicht-konsumtiv</b> erschlossen wurde. Die {_orl_baende()} Bände sind '
...
def jb_register_html(reihe, pers, orte, data):
    ...
    return (f'<h2 id="wer">Genannte Personen</h2>'
            ...
            f'anderes Genre, denn hier stehen sie als Beauftragte der Kommission, nicht als Erzähler ihrer Grabung. '
...
def gesamtregister_page(ner_p, ner_pl, orl_reg, jb, persons):
    ...
    return (f'<h1>Gesamtregister</h1>'
            ...
            f'<a href="orl-register.html">ORL</a> (HathiTrust-Erschließung über {_orl_baende()} Bände) und den '
            ...
            f'normalisierten Namen: eine Zusammenführung für die Anzeige, keine Identitätsaussage, denn derselbe '
...
def netz_page(persons, ner_p, orl_idx, bli, verw, jb, ner_pl, bibls, occ):
    ...
    return (f'<h1>Netzansicht</h1>'
            ...
            f'<div class="note"><p><b>Eine Kante ist keine Beziehung.</b> Drei der vier Quellen verbinden eine '
            f'Person mit einem <i>Druckwerk</i>, nicht mit einem Menschen: dass zwei Namen an denselben Band '
            f'hängen, heißt, dass beide dort vorkommen, nicht dass sie miteinander zu tun hatten. Wer '
            f'Zusammenarbeit sucht, findet sie in der <a href="strecken.html">Streckenzuständigkeit</a>, im '
            f'<a href="organigramm.html">Organigramm</a> und in den Nachlässen des '
            f'<a href="persons.html">Personenregisters</a>, nicht hier. Belastbar ist an diesem Bild die '
            f'<b>Verteilung</b>: welche Namen viele Bände durchziehen und welche an einer einzigen Stelle '
            f'hängen. Nur die Kante <i>bearbeitet</i> (ORL-Faszikel) behauptet eine Urheberschaft.</p></div>'
...
def jb_ereignis_html(pers, kamp):
    ...
    return (f'<h2 id="personalia">Personalia</h2>'
            ...
            f'zur Rezat“), nicht nach Nummern. Es ist dieselbe Gewohnheit, die auch die Titel der '
...
def documentation_page(s):
    ...
    return (
        ...
        f'Reichs-Limeskommission, mit Lebensdaten, Funktion, dem Ort ihres Nachlasses und den Kastellen, die sie ausgruben, '
        ...
        f'<ul>'
        f'<li><a href="register/fundindex.html"><b>Fundindex</b></a>: was gefunden wurde, also Münzen (nach '
        f'Kaisern geordnet), die gängigen Gefäßformen der Terra Sigillata, Ziegelstempel der Truppen und die '
        f'Fundgattungen, jeweils mit genauem Seiten- und Spaltennachweis.</li>'
        ...
        f'<li><a href="register/wortschatz.html"><b>Analyse</b></a>: statistische Blicke auf die Sprache, also wie '
...
def organigramm_page(persons, pname):
    ...
    gridy = 292
    # Die Leitungsebene stand als EIN Kasten mit fünf Namen im Fliesstext: der einzige
    # Kasten des Bildes ohne Verweis, ausgerechnet der mit Mommsen, Sarwey, Hettner,
    # Fabricius und Leonhard. Sie bekommt jetzt je Person einen eigenen, verlinkten Kasten.
    LEITUNG = [("Oscar von Sarwey", "militärischer Dirigent"),
               ("Felix Hettner", "archäologischer Dirigent"),
               ("Ernst Fabricius", "Herausgeber ORL"),
               ("Friedrich Leonhard", "Herausgeber, Schriftführer")]
    ...
    for i, (n, rolle) in enumerate(LEITUNG):
        x = lstartx + i * (lbw + gx)
        S.append(line(x + lbw / 2, lby - 18, x + lbw / 2, lby))
        S.append(box(x, lby, lbw, lbh, [n, rolle], "#e7eef6", href=phref(n), fs=13))
    # Bus zu den Kommissaren
    busy = 262
    S.append(line(cx, lby + lbh, cx, busy))
...
def willkommen_page(s):
    ...
    return (f'<h1>Übersicht</h1>'
            ...
            f'Quellen der <b>Reichs-Limeskommission</b> (1892–1937) liegen hier in fünf Beständen: die laufenden Feldberichte '
...
def orl_spiegel():
    ...
    return (f' Die Endpublikation zeigt das umgekehrte Muster: 1899 erschien keine '
            f'ORL-Lieferung, 1900 dagegen {jahre[1900]} Faszikel, also in dem Jahr, in dem kein '
...
def heft_rhythmus():
    ...
    return (f'<p><b>Der Erscheinungsrhythmus.</b> Aus den Ausgabedaten der Hefte '
            ...
            f'Rhythmus ablesen: bis 1899 im Median <b>{med} Tage</b> von Heft zu Heft, was '
...
def main():
    ...
    # Die Kapsel-Auswertungen mit ausliefern: sie sind bisher nur Bau-Eingabe, die Methodenseite
